...
- **Brush annotation (auto-assist)**: on the annotate page the brush opens by default; click inside the circle and a detection under the cursor (model floor 0.05) becomes an annotation with the model's combined 4×9 class when it matches a project class — otherwise the currently selected class is used; geometry is snapped; four-corner pose projects also store the model corners as four visible keypoints; scroll on the canvas changes brush size, and `B` closes brush mode
